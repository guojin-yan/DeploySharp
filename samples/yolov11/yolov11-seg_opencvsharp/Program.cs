...
namespace yolov11_seg_opencvsharp
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            // 将下面的模型路径替换为你自己的模型路径
            string modelPath = @"E:\Model\Yolo\yolo11s-seg.onnx";
            // 将下面的图片路径替换为你自己的图片路径
            string imagePath = @"E:\Data\image\bus.jpg";

            Yolov11SegConfig config = new Yolov11SegConfig(modelPath);
            //config.SetTargetInferenceBackend(InferenceBackend.OnnxRuntime);
            Yolov11SegModel model = new Yolov11SegModel(config);
