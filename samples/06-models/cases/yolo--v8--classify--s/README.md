...
Model ID: yolo/v8/classify/s
Task: image-classification
Artifact variants: onnx.fp32

## Complete case workflow

Catalog selection and classification package verification; image preprocessing and top-k decoding are part of the workflow. The case stages are: select the immutable catalog entry, verify the ModelPack and asset identities, prepare the declared input, create the compatible backend session, decode the task result, and write an owned output.

Run:

```powershell
dotnet run --project samples/06-models/catalog-workflow/ModelFactoryCatalogInspection.csproj -c Release -- --model-id yolo/v8/classify/s
```
...
~~~powershell
pwsh -NoProfile -File eng/model-catalog/Test-PublishedModelCases.ps1 -ModelId 'yolo/v8/classify/s' -UpdateReadmes
~~~
...
- onnx.fp32: local-ort-openvino-real-image-matrix-verified; preprocessing=ultralytics-letterbox-or-task-specific-v1
