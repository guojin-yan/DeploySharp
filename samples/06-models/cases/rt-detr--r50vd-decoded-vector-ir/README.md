...
Model ID: rt-detr/r50vd-decoded-vector-ir
Task: object-detection
Artifact variants: openvino-ir.fp32

## Complete case workflow

Catalog selection and OpenVINO IR package verification; use the OpenVINO backend workflow for inference. The case stages are: select the immutable catalog entry, verify the ModelPack and asset identities, prepare the declared input, create the compatible backend session, decode the task result, and write an owned output.
...
~~~powershell
pwsh -NoProfile -File eng/model-catalog/Test-PublishedModelCases.ps1 -ModelId 'rt-detr/r50vd-decoded-vector-ir' -UpdateReadmes
~~~
...
- openvino-ir.fp32: local-openvino-real-image-field-verified; preprocessing=paddledetection-rgb-resize-div255-aux-v2; postprocessing=paddledetection-decoded-rows-vector-count-v2; golden=bus.jpg strict-score-0.4 result-count-5; ORT/OpenVINO field parity within declared tolerance
