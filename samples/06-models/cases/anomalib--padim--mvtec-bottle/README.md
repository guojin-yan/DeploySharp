...
Model ID: anomalib/padim/mvtec-bottle
Task: anomaly-detection
Artifact variants: onnx.fp32

## Complete case workflow

Published release inference; writes a source-space anomaly mask. The case stages are: select the immutable catalog entry, verify the ModelPack and asset identities, prepare the declared input, create the compatible backend session, decode the task result, and write an owned output.
...
~~~powershell
pwsh -NoProfile -File eng/model-catalog/Test-PublishedModelCases.ps1 -ModelId 'anomalib/padim/mvtec-bottle' -UpdateReadmes
~~~
...
- onnx.fp32: local-ort-openvino-four-output-contract-verified; preprocessing=anomalib-export-transform-rgb-scale-v1; postprocessing=anomalib-four-output-probability-v1
