...
Model ID: rt-detr/r50vd-decoded-vector-onnx
...
```powershell
dotnet run --project samples/06-models/catalog-workflow/ModelFactoryCatalogInspection.csproj -c Release -- --model-id rt-detr/r50vd-decoded-vector-onnx
```
...
~~~powershell
pwsh -NoProfile -File eng/model-catalog/Test-PublishedModelCases.ps1 -ModelId 'rt-detr/r50vd-decoded-vector-onnx' -UpdateReadmes
~~~
...
- onnx.fp32: local-ort-real-image-field-verified; preprocessing=paddledetection-rgb-resize-div255-aux-v2; postprocessing=paddledetection-decoded-rows-vector-count-v2; golden=bus.jpg strict-score-0.4 result-count-5; ORT/OpenVINO field parity within declared tolerance
