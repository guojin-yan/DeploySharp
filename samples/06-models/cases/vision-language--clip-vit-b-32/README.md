...
Model ID: vision-language/clip-vit-b-32
Task: vision-language-embedding
Artifact variants: dual-encoder.onnx.fp32

## Complete case workflow

Catalog selection and dual-encoder package verification; image/text inputs are required for embedding inference. The case stages are: select the immutable catalog entry, verify the ModelPack and asset identities, prepare the declared input, create the compatible backend session, decode the task result, and write an owned output.

Run:

```powershell
dotnet run --project samples/06-models/catalog-workflow/ModelFactoryCatalogInspection.csproj -c Release -- --model-id vision-language/clip-vit-b-32
```
...
~~~powershell
pwsh -NoProfile -File eng/model-catalog/Test-PublishedModelCases.ps1 -ModelId 'vision-language/clip-vit-b-32' -UpdateReadmes
~~~
...
- dual-encoder.onnx.fp32: ort-openvino-official-golden-verified; preprocessing=official-rgb-pillow-bicubic-shortest-edge-center-crop-224
