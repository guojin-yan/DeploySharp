...
namespace yolov5_seg_imagesharp
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            // 将下面的模型路径替换为你自己的模型路径
            string modelPath = @"E:\Model\Yolo\yolov5s-seg.onnx";
            // 将下面的图片路径替换为你自己的图片路径
            string imagePath = @"E:\Data\image\bus.jpg";

            Yolov5SegConfig config = new Yolov5SegConfig(modelPath);
            //config.SetTargetInferenceBackend(InferenceBackend.OnnxRuntime);
            Yolov5SegModel model = new Yolov5SegModel(config);
            ...
            resultImg.Save(@$"./result_{ModelType.YOLOv5Seg.ToString()}.jpg");
        }
    }
}
