...
internal static class Program
{
    ...
    {
        var modelId = new ModelId("consumer/onnxruntime-classification");
        var artifact = new ModelArtifact(modelId, "onnx", Path.Combine(AppContext.BaseDirectory, "classification.onnx"), preferredBackend: OnnxRuntimeBackendProvider.BackendId);
        ...
        {
            InferenceOutputs outputs = session.Run(InferenceInputs.Create("images", CreateTensor()), CancellationToken.None);
        ...
        var profile = new VisualModelProfile(
            "consumer/onnxruntime-classification.v1", modelId, VisualTaskId.ImageClassification, "1.0", "onnx",
        ...
        Console.WriteLine("DEPLOYSHARP_ONNXRUNTIME_CONSUMER_OK");
        return 0;
    }

    private static Tensor<float> CreateTensor()
    {
        return new Tensor<float>(new TensorShape(1, 3, 2, 2), new[]
        {
            1f, 1f, 1f, 1f,
            2f, 2f, 2f, 2f,
            3f, 3f, 3f, 3f
