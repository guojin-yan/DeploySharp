...
using static DeploySharp.Data.VisualizeHandler;
    ...
    public partial class MainForm : Form
    {
        ...
        private void buttonInfer_Click(object sender, EventArgs e)
        {
            ...
            using (var image = Cv2.ImRead(imagePath))
            {
                ...
                using Mat resultImg = visualizeHandler.ExecuteDrawing(result, image as Mat, new VisualizeOptions(1.0f));

                using Mat showImg = CvDataProcessor.Resize(resultImg, new Data.Size(pictureBox1.Width, pictureBox1.Height), ImageResizeMode.Pad);
                // 3. 将ImageSharp图像转换为Windows Forms可用的Bitmap
                using (var memoryStream = new MemoryStream())
                {
                    BitmapConverter.ToBitmap(showImg).Save(memoryStream, System.Drawing.Imaging.ImageFormat.Bmp);
