...
The default image is <code>E:\\Data\\ocr\\demo\\_1.jpg</code>. On the current workstation that name resolves to the existing file <code>E:\\Data\\ocr\\demo_1.jpg</code>; set <code>DEPLOYSHARP_PADDLEOCR_IMAGE</code> to use another image. The complete-pipeline CSV is written as <code>paddleocr-full-*.csv</code>. In addition to stage means, the final row contains <code>total_p50_ms</code> and <code>total_p95_ms</code> calculated from the timed calls; these are zero only for an internal single-sample autotune probe and are populated by the formal run.
