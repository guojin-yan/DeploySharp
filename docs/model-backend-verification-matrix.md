...
UVDoc OpenCV DNN 精确组合已尝试但失败：OpenCV 5.0 importer 在 `PaddingLayerImpl::forward` 抛出 `inputs[0].dims == 4`，矩阵标记为 `✗`。TensorRT 11 则已从同一 ONNX 构建并加载 engine，`trtexec` 640x640 推理通过，GPU compute mean/P95 `8.55564/9.9389 ms`；该记录证明 TensorRT engine 可执行，DeploySharp bridge decoder/tensor parity 仍待专用测试。
