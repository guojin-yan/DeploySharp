...
Stage 19 adds artifact-bound execution contracts for `PaddleOcrDet`, `PaddleOcrRec`, `AnomalibSeg`, and `BriaRmbg` in the existing `JYPPX.DeploySharp.Visual` package. Image preparation remains optional in `JYPPX.DeploySharp.Visual.OpenCV`; ONNX Runtime and OpenVINO receive only named Core tensors. Runtime NuGet packages remain code-only; the public PP-OCRv5 model, dictionary, and Apache-2.0 notice are available as immutable assets in the [PP-OCRv5 public Release](https://github.com/guojin-yan/DeploySharp/releases/tag/models-20260818.ppocrv5.1). / 阶段 19 在现有 `JYPPX.DeploySharp.Visual` 包中增加 `PaddleOcrDet`、`PaddleOcrRec`、`AnomalibSeg` 与 `BriaRmbg` 的工件绑定执行合同。图像准备继续作为 `JYPPX.DeploySharp.Visual.OpenCV` 的可选能力；ONNX Runtime 与 OpenVINO 只接收具名 Core tensor。运行时 NuGet 包仍然只包含代码；PP-OCRv5 模型、字典和 Apache-2.0 归属文件已作为不可变资产发布在公开 Release 中。
...
The official PaddleOCR JavaScript model assets point to six `*_onnx_infer.tar` archives on the Paddle model-ecology host. Their `inference.onnx` and `inference.yml` entries, archive sizes, and SHA-256 values are recorded in [`paddleocr-release-admission.json`](https://github.com/guojin-yan/DeploySharp/blob/DeploySharpV2.0/eng/models/ocr-anomaly-rmbg/paddleocr-release-admission.json) under `officialOnnxInferenceArchives`; the same evidence pins `ppocrv5_dict.txt` to the raw PaddleOCR file at the revision above. These archives do not contain a DeploySharp ModelPack manifest. Their ONNX bytes are also not the current immutable Release identities (for example, mobile-cls is `94a6a0...` in the official archive versus `dd8b2b...` in the current ModelPack). ModelFactory therefore remains bound to the existing DeploySharp Release assets until a separately generated and published ModelPack is available; replacing a catalog URL with an official archive URL alone would fail manifest/model identity validation. / 官方 PaddleOCR JavaScript 模型资源指向 Paddle 模型生态主机上的六个 `*_onnx_infer.tar` 归档。其 `inference.onnx`、`inference.yml`、归档大小和 SHA-256 已记录在上述证据文件的 `officialOnnxInferenceArchives`；同一证据将 `ppocrv5_dict.txt` 固定到上述 revision 的 PaddleOCR 原始文件。这些归档不包含 DeploySharp ModelPack manifest，且内部 ONNX 字节也不是当前不可变 Release 的身份（例如 mobile-cls 官方归档为 `94a6a0...`，当前 ModelPack 为 `dd8b2b...`）。因此在生成并发布独立 ModelPack 之前，ModelFactory 继续绑定现有 DeploySharp Release；只替换 catalog 下载 URL 会被 manifest/model identity 校验拒绝。

BRIA references the official [RMBG-1.4 model card](https://huggingface.co/briaai/RMBG-1.4) and [RMBG-2.0 model card](https://huggingface.co/briaai/RMBG-2.0). The original candidate manifests under `eng/models/ocr-anomaly-rmbg/manifests` retain their `External` provenance record, while the directly published Release manifests under `eng/models/ocr-anomaly-rmbg/releases` are cataloged as downloadable `Preview` entries for RMBG 1.4 and RMBG 2.0 fp32/dynamic-int8. Exact local ONNX export provenance and processor revision remain unproven, so these entries are not `AlgorithmVerified`. / BRIA 引用官方 RMBG-1.4 与 RMBG-2.0 模型卡。`eng/models/ocr-anomaly-rmbg/manifests` 下的原始候选清单保留 `External` 来源记录，而 `eng/models/ocr-anomaly-rmbg/releases` 下直接发布的清单已作为可下载 `Preview` 条目收录，包括 RMBG 1.4 以及 RMBG 2.0 fp32/dynamic-int8。精确本地 ONNX 导出来源与 processor revision 仍未证明，因此这些条目不是 `AlgorithmVerified`。
