...
BRIA references the official [RMBG-1.4 model card](https://huggingface.co/briaai/RMBG-1.4) and [RMBG-2.0 model card](https://huggingface.co/briaai/RMBG-2.0). Exact local ONNX export provenance, checkpoint terms, and processor revision are not independently proven. Therefore BRIA manifests under `eng/models/ocr-anomaly-rmbg/manifests` remain `External` and are kept out of the published catalog. Local execution is `ContractVerified + LocalBackendVerified`, not `AlgorithmVerified`. / BRIA 引用官方 RMBG-1.4 与 RMBG-2.0 模型卡。精确本地 ONNX 导出来源、checkpoint 条款与 processor revision 尚未独立证明。因此 `eng/models/ocr-anomaly-rmbg/manifests` 下的 BRIA 清单保持 `External`，不进入已发布 catalog。本地执行是 `ContractVerified + LocalBackendVerified`，不是 `AlgorithmVerified`。
