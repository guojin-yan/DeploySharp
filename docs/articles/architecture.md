...
Visual receives the output of an external image adapter through `PreparedVisualInput`. The official future adapter is `JYPPX.DeploySharp.Visual.OpenCV`; alternative adapters remain separate and never multiply with backend packages. / Visual 通过 `PreparedVisualInput` 接收外部图像适配器的输出。未来官方适配器为 `JYPPX.DeploySharp.Visual.OpenCV`；其他适配器保持独立，不与后端组合成乘积包。
