...
ModelPack validates ONNX plus IR XML/BIN sizes and SHA256. ModelFactory has offline Preview selection evidence for `onnx + onnxruntime/openvino` and `openvino-ir + openvino`. The embedded official catalog remains empty and no GitHub Release asset is created. / ModelPack 校验 ONNX 与 IR XML/BIN 的大小和 SHA256。ModelFactory 提供 `onnx + onnxruntime/openvino` 与 `openvino-ir + openvino` 的离线 Preview 选择证据。内置官方目录保持为空，也未创建 GitHub Release 资产。
