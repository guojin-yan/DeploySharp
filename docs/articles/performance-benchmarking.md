...
本页说明 DeploySharp 的统一测速方法。它用于比较同一模型、同一输入和同一设备上的后端差异，不把一次小型基准图的结果当成所有模型的性能承诺。具名设备、模型工件和结果统一登记在[设备性能实测](device-performance-benchmarks.md)，新增记录应沿用该页的字段和状态语义。
