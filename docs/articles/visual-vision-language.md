...
The separate CLIP ViT-B/32 release manifest is `redistributionAllowed:true` and available as a ModelFactory Preview in the [shared vision collection](models-vision-collection.md). The historical CLIP development manifest, SigLIP, and SigLIP 2 remain External and are not `AlgorithmVerified`; SigLIP 2 still lacks a complete audited ONNX/OpenVINO dual-encoder export and native processor fidelity evidence. / 独立 CLIP ViT-B/32 发布清单允许再分发，并作为 ModelFactory Preview 收录到[共享视觉模型集合](models-vision-collection.md)。历史 CLIP 开发清单、SigLIP 与 SigLIP 2 继续保持 External，且均不宣称 `AlgorithmVerified`。
