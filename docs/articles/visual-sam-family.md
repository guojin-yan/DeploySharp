...
All three model manifests are `External`, `redistributionAllowed:false`, not `AlgorithmVerified`, and excluded from the official catalog. See `eng/models/sam/sam-family-support.json`; local paths are never serialized into a public catalog or package. / 三个清单均为 External、禁止再分发、非 AlgorithmVerified 且不进入官方目录；本机路径不写入公开 catalog 或包。
