...
The wrapper now has a GitHub prerelease tagged `v5.0.0-preview.1`, published on 2026-08-05. It is still not a stable release. Consumers must use the exact managed/runtime preview pair and must not infer stable compatibility from the existence of a prerelease tag. / 包装器现已有 2026-08-05 发布的 GitHub prerelease 标签 `v5.0.0-preview.1`，但仍不是稳定版。用户必须使用精确匹配的托管/runtime preview 组合，不得仅因存在 prerelease 标签就推断稳定兼容性。
