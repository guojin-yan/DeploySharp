...
| Model/vocabulary/config | external files, no RID inference | caller / ModelFactory only after publication approval |
...
See [the acquisition article](model-acquisition-blip-family.md), [all-stage inventory](development-model-inventory.md), and `eng/models/generative-vision-language/generative-vision-language-family-support.json` for supply-chain and publication status. / 供应链与发布状态见获取文章、全阶段清单和结构化支持文件。
