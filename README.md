...
`2.0.0-alpha.1` currently adds a complete backend-neutral Visual anomaly detection/segmentation slice: explicit score/map semantics, owned results, ONNX Runtime and OpenVINO CPU execution, OpenCV image input, ModelPack/ModelFactory evidence, tests, docs, packaging and a clean consumer. See the [detailed release note](docs/releases/2.0.0-alpha.1.md) and [version index](docs/releases/README.md). / 当前 `2.0.0-alpha.1` 新增完整的后端无关 Visual 异常检测/分割切片，包括显式分数/异常图语义、自有结果、ONNX Runtime 与 OpenVINO CPU 执行、OpenCV 图像输入、ModelPack/ModelFactory 证据、测试、文档、打包及 clean consumer。详见[本版本说明](docs/releases/2.0.0-alpha.1.md)与[版本索引](docs/releases/README.md)。
