...
namespace JYPPX.DeploySharp.Visual.Models.Yolo
{
    ...
    public sealed class YoloDetectionDecoder : IVisualDecoder
    {
        ...
        public object Decode(VisualDecodeContext context)
        {
            ...
            if (context.Input.BatchSize != 1) throw Invalid("YOLO detection currently requires batch size one.", context);
            ITensor tensor;
            ...
            if (Contract.IsEndToEnd) return DecodeEndToEnd(context, tensor.Shape, values);
            return DecodeRaw(context, tensor.Shape, values);
        }

        private DetectionResult DecodeRaw(VisualDecodeContext context, TensorShape shape, float[] values)
        {
            int candidates;
            int fields;
            ...
            if (fields != Contract.FieldCount || candidates <= 0 || values.LongLength != checked((long)candidates * fields)) throw Invalid("The raw YOLO tensor shape does not match its explicit field contract.", context, details: shape.ToString());
            ...
            for (int candidateIndex = 0; candidateIndex < candidates; candidateIndex++)
            {
                ...
                float objectness = Contract.HasObjectness ? Score(Read(values, candidates, fields, candidateIndex, 4, attributeMajor), context, candidateIndex, "objectness") : 1f;
                ...
                {
                    int bestClass = 0;
                    float bestClassScore = Score(Read(values, candidates, fields, candidateIndex, classOffset, attributeMajor), context, candidateIndex, "class-score");
                    for (int classIndex = 1; classIndex < Contract.ClassCount; classIndex++)
                    {
                        float classScore = Score(Read(values, candidates, fields, candidateIndex, classOffset + classIndex, attributeMajor), context, candidateIndex, "class-score");
                        if (classScore > bestClassScore) { bestClassScore = classScore; bestClass = classIndex; }
                    }
                    AddRawCandidate(decoded, context, values, candidates, fields, candidateIndex, attributeMajor, bestClass, objectness * bestClassScore);
                }
                else
                {
                    ...
                    {
                        float classScore = Score(Read(values, candidates, fields, candidateIndex, classOffset + classIndex, attributeMajor), context, candidateIndex, "class-score");
                        AddRawCandidate(decoded, context, values, candidates, fields, candidateIndex, attributeMajor, classIndex, objectness * classScore);
                    }
                }
            }
        ...
        private DetectionResult DecodeEndToEnd(VisualDecodeContext context, TensorShape shape, float[] values)
        {
            int fields = Contract.FieldCount;
            int rows;
            if (shape.Rank == 3 && shape[0] == 1 && shape[2] == fields) rows = checked((int)shape[1]);
            ...
            if (rows < 0 || values.LongLength != checked((long)rows * fields)) throw Invalid("The end-to-end YOLO tensor element count is inconsistent.", context, details: shape.ToString());
            ...
            for (int row = 0; row < rows && results.Count < Options.MaximumDetections; row++)
            {
                ...
                {
                    int batch = Integer(values[offset], context, row, "batch-index");
                    if (batch != 0) continue;
                    boxOffset = offset + 1;
        ...
        private void AddRawCandidate(List<VisualDetectionCandidate> decoded, VisualDecodeContext context, float[] values, int candidates, int fields, int candidateIndex, bool attributeMajor, int classIndex, float score)
        {
            ...
            try
            {
                modelBox = DetectionPostprocessing.DecodeModelBox(
                    ...
                    false,
                    context.Input.ModelSize,
                    Read(values, candidates, fields, candidateIndex, 0, attributeMajor),
                    Read(values, candidates, fields, candidateIndex, 1, attributeMajor),
                    Read(values, candidates, fields, candidateIndex, 2, attributeMajor),
                    Read(values, candidates, fields, candidateIndex, 3, attributeMajor));
            }
        ...
        private static float Read(float[] values, int candidates, int fields, int candidate, int field, bool attributeMajor)
        {
