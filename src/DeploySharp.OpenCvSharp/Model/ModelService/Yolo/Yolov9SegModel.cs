...
namespace DeploySharp.Model
{
    ...
    /// <remarks>
    /// <para>
    /// YOLOv9 Seg combines the Programmable Gradient Information (PGI) architecture with
    /// instance segmentation capabilities, providing state-of-the-art pixel-level object segmentation.
    /// YOLOv9 Seg将可编程梯度信息(PGI)架构与实例分割能力相结合，提供最先进的像素级目标分割。
    /// </para>
    /// <para>
    /// Key features:
    /// 主要特点：
    ...
    ///   像素级实例分割掩码
    /// - PGI-based architecture for improved training
    ///   基于PGI的架构以改进训练
    ...
    /// <example>
    /// <code>
    /// // Create YOLOv9 segmentor
    /// // 创建YOLOv9分割器
    /// var config = new Yolov9SegConfig("yolov9n-seg.onnx")
    /// {
    ...
    /// };
    /// 
    /// using (var segmentor = new Yolov9SegModel(config))
    /// {
    ...
    /// <seealso cref="Yolov9DetModel"/>
    public class Yolov9SegModel : IYolov9SegModel
    {
        /// <summary>
        /// Creates a new YOLOv9 segmentation model instance.
        /// 创建新的YOLOv9分割模型实例。
        /// </summary>
        ...
        /// <exception cref="InvalidCastException">Thrown when config is not Yolov9SegConfig / 当config不是Yolov9SegConfig时抛出</exception>
        /// <remarks>
        ...
        /// <summary>
        /// Preprocesses image for YOLOv9 segmentation inference.
        /// 对图像进行预处理以进行YOLOv9分割推理。
        /// </summary>
        /// <param name="img">Input image (OpenCvSharp Mat) / 输入图像(OpenCvSharp Mat)</param>
        ...
        /// <summary>
        /// Preprocesses batch of images for YOLOv9 segmentation inference.
        /// 对批量图像进行预处理以进行YOLOv9分割推理。
        /// </summary>
        /// <param name="imgs">List of input images / 输入图像列表</param>
