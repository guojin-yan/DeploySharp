...
namespace DeploySharp.Model
{
    ...
    /// <remarks>
    /// <para>
    /// YOLOv9 introduces the Programmable Gradient Information (PGI) and Generalized ELAN (GELAN)
    /// architecture, representing a significant advancement in object detection technology.
    /// YOLOv9引入了可编程梯度信息(PGI)和广义ELAN(GELAN)架构，代表了目标检测技术的重大进步。
    /// </para>
    /// <para>
    /// Key features:
    /// 主要特点：
    /// - Programmable Gradient Information (PGI) for better training
    ///   可编程梯度信息(PGI)以获得更好的训练效果
    /// - Generalized ELAN (GELAN) architecture
    ///   广义ELAN(GELAN)架构
    ...
    /// <example>
    /// <code>
    /// // Create YOLOv9 detector
    /// // 创建YOLOv9检测器
    /// var config = new Yolov9DetConfig("yolov9n.onnx")
    /// {
    ...
    /// };
    /// 
    /// using (var detector = new Yolov9DetModel(config))
    /// {
    ...
    /// <seealso cref="Yolov9SegModel"/>
    public class Yolov9DetModel : IYolov9DetModel
    {
        /// <summary>
        /// Creates a new YOLOv9 detection model instance.
        /// 创建新的YOLOv9检测模型实例。
        /// </summary>
        ...
        /// <exception cref="InvalidCastException">Thrown when config is not Yolov9DetConfig / 当config不是Yolov9DetConfig时抛出</exception>
        /// <remarks>
        ...
        /// <summary>
        /// Preprocesses image for YOLOv9 detection inference.
        /// 对图像进行预处理以进行YOLOv9检测推理。
        /// </summary>
        /// <param name="img">Input image (OpenCvSharp Mat) / 输入图像(OpenCvSharp Mat)</param>
        ...
        /// <summary>
        /// Preprocesses batch of images for YOLOv9 detection inference.
        /// 对批量图像进行预处理以进行YOLOv9检测推理。
        /// </summary>
        /// <param name="imgs">List of input images / 输入图像列表</param>
