...
{
    /// <summary>
    /// YOLOv6 object detection model implementation.
    /// YOLOv6目标检测模型实现。
    /// </summary>
    /// <remarks>
    /// <para>
    /// YOLOv6 is a single-stage object detection framework optimized for industrial deployment,
    /// featuring hardware-friendly network design and efficient inference.
    /// YOLOv6是专为工业部署优化的单阶段目标检测框架，具有硬件友好的网络设计和高效推理。
    /// </para>
    /// <para>
    /// Key features:
    /// 主要特点：
    ...
    ///   硬件友好的高效骨干网络
    /// - Bi-directional concatenation (BiC) module
    ///   双向连接(BiC)模块
    /// - Anchor-aided training with anchor-free inference
    ///   带锚辅助训练的无锚推理
    ...
    /// <example>
    /// <code>
    /// // Create YOLOv6 detector
    /// // 创建YOLOv6检测器
    /// var config = new Yolov6DetConfig("yolov6n.onnx")
    /// {
    ...
    /// };
    /// 
    /// using (var detector = new Yolov6DetModel(config))
    /// {
    ...
    {
        /// <summary>
        /// Creates a new YOLOv6 detection model instance.
        /// 创建新的YOLOv6检测模型实例。
        /// </summary>
        ...
        /// <exception cref="InvalidCastException">Thrown when config is not Yolov6DetConfig / 当config不是Yolov6DetConfig时抛出</exception>
        /// <remarks>
        ...
        /// <summary>
        /// Preprocesses image for YOLOv6 detection inference.
        /// 对图像进行预处理以进行YOLOv6检测推理。
        /// </summary>
        /// <param name="img">Input image (OpenCvSharp Mat) / 输入图像(OpenCvSharp Mat)</param>
        ...
        /// <summary>
        /// Preprocesses batch of images for YOLOv6 detection inference.
        /// 对批量图像进行预处理以进行YOLOv6检测推理。
        /// </summary>
        /// <param name="imgs">List of input images / 输入图像列表</param>
