...
namespace DeploySharp.Model
{
    ...
    /// <remarks>
    /// <para>
    /// YOLOv5 is a popular object detection model known for its balance of speed and accuracy.
    /// It uses anchor-based detection with CSPDarknet backbone and PANet neck.
    /// YOLOv5是一个流行的目标检测模型，以其速度和准确性的平衡而闻名。
    /// 它使用基于锚点的检测，具有CSPDarknet骨干网络和PANet颈部。
    /// </para>
    /// <para>
    /// Available variants:
    /// 可用变体：
    /// - YOLOv5n (nano): Smallest, fastest / 最小，最快
    /// - YOLOv5s (small): Good balance / 良好的平衡
    /// - YOLOv5m (medium): Better accuracy / 更好的准确性
    /// - YOLOv5l (large): High accuracy / 高准确性
    /// - YOLOv5x (xlarge): Highest accuracy / 最高准确性
    /// </para>
    /// </remarks>
    /// <example>
    /// <code>
    ...
    /// };
    /// 
    /// using (var detector = new Yolov5DetModel(config))
    /// {
    ...
    /// </code>
    /// </example>
    /// <seealso cref="Yolov5DetConfig"/>
    /// <seealso cref="Yolov5SegModel"/>
    public class Yolov5DetModel : IYolov5DetModel
    {
        /// <summary>
        /// Creates a new YOLOv5 detection model instance.
        /// 创建新的YOLOv5检测模型实例。
        /// </summary>
        ...
        /// <exception cref="InvalidCastException">Thrown when config is not Yolov5DetConfig / 当config不是Yolov5DetConfig时抛出</exception>
        /// <remarks>
        ...
        ///         Cv2.PutText(image, det.Category, new Point(det.Bounds.X, det.Bounds.Y - 5),
        ...
        /// <summary>
        /// Preprocesses image for YOLOv5 detection inference.
        /// 对图像进行预处理以进行YOLOv5检测推理。
        /// </summary>
        /// <param name="img">Input image (OpenCvSharp Mat) / 输入图像(OpenCvSharp Mat)</param>
