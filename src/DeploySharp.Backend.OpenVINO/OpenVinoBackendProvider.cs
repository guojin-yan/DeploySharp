...
namespace JYPPX.DeploySharp.Backends.OpenVINO
{
    ...
    public sealed class OpenVinoBackendProvider : IBackendProvider
    {
        ...
        /// <summary>Validates, reads, and compiles a local ONNX or IR model into a caller-owned session. / 验证、读取并编译本地 ONNX 或 IR 模型，返回调用方持有的会话。</summary>
        public IInferenceSession CreateSession(ModelArtifact artifact, BackendRequest request, CoreSessionOptions options)
        {
            ...
            if (_options.RequestCount.HasValue && _options.RequestCount.Value < options.MaxConcurrency)
                throw new OpenVinoBackendException(OpenVinoErrorCodes.ConfigurationInvalid, "RequestCount cannot be lower than SessionOptions.MaxConcurrency.", modelId: artifact.ModelId, operation: "configure", device: _options.Device);
            ...
            try
            {
                ...
                var session = new OpenVinoSession(artifact, core, model, compiled, options.MaxConcurrency, _options.AllowDynamicShapes, _options.Device);
                core = null;
                model = null;
                compiled = null;
                return session;
