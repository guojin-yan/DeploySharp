                ...
                if (conf > 0.25)  // 置信度阈值过滤
                {
                    for (int j = 5; j < outputSize1; j++)  // 遍历每个类别
                    {
                        ...
                        if (conf1 > 0.2)  // 置信度阈值过滤
                        {
                            ...
                            masks.Add(Mat.FromPixelData(1, dataTensor[1].Shape[1], MatType.CV_32FC1, Marshal.UnsafeAddrOfPinnedArrayElement(result0, (outputSize1 * i + outputSize1 - dataTensor[1].Shape[1]))));
                        }
                    }
                }
