...
{
    /// <summary>
    /// Configuration class specifically designed for  IFDETRDet detection models
    /// 专为 IFDETRDet目标检测模型设计的配置类
    /// </summary>
    /// <remarks>
    /// <para>
    /// Presets optimal default values for  IFDETRDet detection models while allowing customization.
    /// Inherits all YOLO-specific configuration parameters from <see cref="IImgConfig"/>.
    /// </para>
    /// <para>
    /// 为 IFDETRDet检测模型预设了最优默认值，同时允许自定义配置。
    /// 从<see cref="IImgConfig"/>继承了所有IImgConfig特定的配置参数。
    /// </para>
    /// <example>
    ...
    /// <code>
    /// var config = new IFDETRDetConfig(
