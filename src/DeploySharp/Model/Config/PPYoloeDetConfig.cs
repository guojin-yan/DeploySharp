...
{
    /// <summary>
    /// Configuration class specifically designed for PPYoloe detection models
    /// 专为PPYoloe目标检测模型设计的配置类
    /// </summary>
    /// <remarks>
    /// <para>
    /// Presets optimal default values for PPYoloe detection models while allowing customization.
    /// Inherits all YOLO-specific configuration parameters from <see cref="YoloConfig"/>.
    /// </para>
    /// <para>
    /// 为PPYoloe检测模型预设了最优默认值，同时允许自定义配置。
    /// 从<see cref="YoloConfig"/>继承了所有YOLO特定的配置参数。
    /// </para>
    /// <example>
    /// Basic initialization:
    /// <code>
    /// var config = new PPYoloeDetConfig("yolov5s.onnx")
    /// {
    ...
    /// <code>
    /// var config = new PPYoloeDetConfig(
    ...
    /// </example>
    /// </remarks>
    public class PPYoloeDetConfig : YoloConfig
    {
        /// <summary>
