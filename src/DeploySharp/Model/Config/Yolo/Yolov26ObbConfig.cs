...
{
    /// <summary>
    /// Configuration class specifically designed for YOLOv26 oriented bounding box models
    /// 专为YOLOv26旋转框检测模型设计的配置类
    /// </summary>
    /// <remarks>
    /// <para>
    /// Presets optimal default values for YOLOv26 oriented bounding box models while allowing customization.
    /// Inherits all YOLO-specific configuration parameters from <see cref="YoloConfig"/>.
    /// </para>
    /// <para>
    /// 为YOLOv26旋转框检测模型预设了最优默认值，同时允许自定义配置。
    /// 从<see cref="YoloConfig"/>继承了所有YOLO特定的配置参数。
    /// </para>
    /// <example>
    /// Basic initialization:
    /// <code>
    /// var config = new Yolov26ObbConfig("yolov26s-obb.onnx")
    /// {
    ...
    /// <code>
    /// var config = new Yolov26ObbConfig(
    ///     modelPath: "yolov26m.onnx",
    ...
    /// </example>
    /// </remarks>
    public class Yolov26ObbConfig : Yolov8ObbConfig
    {
        /// <summary>
        ...
        /// 使用前需要单独设置模型路径。
        /// </remarks>
        public Yolov26ObbConfig() { }
        /// <summary>
        /// Initializes a new instance with model path and recommended defaults
        /// 使用模型路径和推荐默认值初始化新实例
        /// </summary>
        /// <param name="modelPath">
        /// Path to the YOLOv26 oriented bounding box model file (.onnx/.pt)
        /// YOLOv26 旋转框检测模型模型文件路径 (.onnx/.pt)
        /// </param>
        public Yolov26ObbConfig(string modelPath)
        {
            this.ModelType = ModelType.YOLOv26Obb;
        ...
        /// 可完全自定义配置的高级构造函数
        /// </summary>
        /// <param name="modelPath">Path to oriented bounding box model  旋转框检测模型路径</param>
