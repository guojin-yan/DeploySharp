...
namespace DeploySharp.Model
{
    ...
    /// <remarks>
    /// <para>
    /// Presets optimal default values for YOLOv10 detection models while allowing customization.
    /// Inherits all YOLO-specific configuration parameters from <see cref="YoloConfig"/>.
    /// </para>
    /// <para>
    /// 为YOLOv10检测模型预设了最优默认值，同时允许自定义配置。
    /// 从<see cref="YoloConfig"/>继承了所有YOLO特定的配置参数。
    /// </para>
    /// <example>
    /// Basic initialization:
    /// <code>
    /// var config = new Yolov10DetConfig("yolov10s.onnx")
    /// {
    ...
    /// <code>
    /// var config = new Yolov10DetConfig(
    ...
    /// </example>
    /// </remarks>
    public class Yolov10DetConfig : YoloConfig
    {
        /// <summary>
        ...
        /// 使用前需要单独设置模型路径。
        /// </remarks>
        public Yolov10DetConfig() { }
        /// <summary>
        /// Initializes a new instance with model path and recommended defaults
        /// 使用模型路径和推荐默认值初始化新实例
        /// </summary>
        /// <param name="modelPath">
        /// Path to the YOLOv10 model file (.onnx/.ir)
        /// YOLOv10模型文件路径 (.onnx/.ir)
        /// </param>
        public Yolov10DetConfig(string modelPath) 
        {
            this.ModelType = ModelType.YOLOv10Det;
        ...
        /// 允许完全自定义的参数化构造函数
        /// </summary>
        /// <param name="modelPath">Path to the YOLOv10 model 模型路径</param>
