...
namespace DeploySharp.Model
{
    ...
    /// <summary>
    /// Configuration class specifically designed for YOLOv26 人体姿态估计模型 models
    /// 专为YOLOv26目标检测模型设计的配置类
    /// </summary>
    /// <remarks>
    /// <para>
    /// Presets optimal default values for YOLOv26 人体姿态估计模型 models while allowing customization.
    /// Inherits all YOLO-specific configuration parameters from <see cref="YoloConfig"/>.
    /// </para>
    /// <para>
    /// 为YOLOv26检测模型预设了最优默认值，同时允许自定义配置。
    /// 从<see cref="YoloConfig"/>继承了所有YOLO特定的配置参数。
    /// </para>
    /// <example>
    /// Basic initialization:
    /// <code>
    /// var config = new Yolov26PoseConfig("yolov26s-pose.onnx")
    /// {
    ...
    /// <code>
    /// var config = new Yolov26PoseConfig(
    ///     modelPath: "yolov26s-pose.onnx",
    ...
    /// </example>
    /// </remarks>
    public class Yolov26PoseConfig : Yolov8PoseConfig
    {
        /// <summary>
        ...
        /// 使用前需要单独设置模型路径。
        /// </remarks>
        public Yolov26PoseConfig() { }
        /// <summary>
        /// Initializes a new instance with model path and reasonable defaults
        /// 使用模型路径和合理的默认值初始化新实例
        /// </summary>
        /// <param name="modelPath">
        /// Path to the YOLOv26 human pose estimation model (.onnx/.ir)
        /// YOLOv26人体姿态估计模型路径 (.onnx/.ir)
        /// </param>
        public Yolov26PoseConfig(string modelPath)
        {
            this.ModelType = ModelType.YOLOv26Pose;
