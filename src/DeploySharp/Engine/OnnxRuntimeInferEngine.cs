...
namespace DeploySharp.Engine
{
    public class OnnxRuntimeInferEngine : IModelInferEngine
    {
        ...
        public void LoadModel(ref ModelConfig config)
        {
        ...
        public DataTensor Predict(DataTensor inputs)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < inputs.Count; i++)
                {
                    ...
                    {
                        var inputOrtValue = OrtValue.CreateTensorValueFromMemory((float[])data.Buffer, data.Shape.Select(x => (long)x).ToArray());
                    ...
                    {
                        var inputOrtValue = OrtValue.CreateTensorValueFromMemory((int[])data.Buffer, data.Shape.Select(x => (long)x).ToArray());
                        inputsData.Add(data.Name, inputOrtValue);
                ...
                for (int i = 0; i < outputs.Count; i++)
                {
                    ...
                    if (type == TensorElementType.Float)
                    {
                        dataTensor.AddNode(modelConfig.OutputNames[i],
                            0,
                            TensorType.Output,
                            resultData.GetTensorDataAsSpan<float>().ToArray(),
                    ...
                    else if (type == TensorElementType.Int32)
                    {
                        dataTensor.AddNode(modelConfig.OutputNames[i],
                            0,
                            TensorType.Output,
                            resultData.GetTensorDataAsSpan<int>().ToArray(),
                            shape.Select(x => (int)x).ToArray(),
                            typeof(int));
