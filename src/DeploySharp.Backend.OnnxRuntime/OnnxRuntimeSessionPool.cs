...
{
    /// <summary>Dispatches calls to independently-created ONNX Runtime sessions. / 将调用分发到彼此独立创建的 ONNX Runtime Session。</summary>
    internal sealed class OnnxRuntimeSessionPool : IInferenceSession
    {
        private readonly object _lifetimeGate = new object();
        private readonly object _slotGate = new object();
        private readonly OnnxRuntimeSession[] _sessions;
        private readonly Queue<int> _freeSlots;
        private readonly SemaphoreSlim _available;
        private readonly CancellationTokenSource _disposeSource = new CancellationTokenSource();
        private bool _disposed;

        public OnnxRuntimeSessionPool(IReadOnlyList<OnnxRuntimeSession> sessions)
        {
            if (sessions == null) throw new ArgumentNullException(nameof(sessions));
            if (sessions.Count < 2) throw new ArgumentException("An ONNX Runtime session pool requires at least two independent sessions.", nameof(sessions));
        ...
        public void Dispose()
        {
            ...
            try
            {
                ...
                foreach (OnnxRuntimeSession session in _sessions)
                {
                    try { session.Dispose(); }
        ...
        private int AcquireSlot(CancellationToken cancellationToken)
        {
            ...
            {
                throw new OnnxRuntimeBackendException(OnnxRuntimeErrorCodes.Cancelled, "ONNX Runtime session-pool acquisition was cancelled.", exception, operation: "pool-acquire");
            }
            lock (_slotGate)
            {
                EnsureUsable();
        ...
        private CancellationToken CreateOperationToken(CancellationToken callerToken, out CancellationTokenSource? linked)
        {
            lock (_lifetimeGate)
            {
                if (_disposed) throw new OnnxRuntimeBackendException(OnnxRuntimeErrorCodes.ObjectDisposed, "The ONNX Runtime session pool has been disposed.", operation: "pool");
