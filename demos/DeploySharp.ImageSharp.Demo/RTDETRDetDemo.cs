...
{
    public class RTDETRDetDemo
    {
        public static void Run()
        {
            // 模型和测试图片可以前往QQ群(945057948)下载
            // 将下面的模型路径替换为你自己的模型路径
            string modelPath = @"E:\Model\RTv4-S-hgnet.onnx";
            // 将下面的图片路径替换为你自己的图片路径
            string imagePath = @"E:\Data\image\bus.jpg";

            RTDETRDetConfig config = new RTDETRDetConfig(modelPath);
            ...
            RTDETRDetModel model = new RTDETRDetModel(config);
            ...
            resultImg.Save(@$"./result_{ModelType.RTDETRDet.ToString()}.jpg");
        }
    }
}
